    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 bg-gray-50 text-gray-800 font-sans">
            <header className="bg-white shadow px-6 py-4 flex justify-between items-center rounded-xl shrink-0">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <select 
                        ...
                        className="border-gray-300 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 py-1.5 px-3 border bg-white cursor-pointer"
                    >
                ...
                <div className="flex-1 flex flex-col gap-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
                        <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col justify-center">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Global Avg Temp</h3>
                        ...
                        <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col justify-center">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Hottest Country</h3>
                            <p className="text-xl font-bold text-red-600 mt-1 truncate">{hottest.entity}</p>
                            <p className="text-sm text-gray-500 font-semibold">{hottest.temp ? hottest.temp.toFixed(2) + ' °C' : '-'}</p>
                        </div>
                        <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col justify-center">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Coldest Country</h3>
                            <p className="text-xl font-bold text-blue-600 mt-1 truncate">{coldest.entity}</p>
                            <p className="text-sm text-gray-500 font-semibold">{coldest.temp ? coldest.temp.toFixed(2) + ' °C' : '-'}</p>
                        </div>
                    </div>

                    <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex-1 flex flex-col min-h-[400px]">
                        <h2 className="text-lg font-bold text-gray-700 mb-2">Global Average Temperature Map ({selectedYear})</h2>
                        <Map 
                            state={state} 
                        ...
                        />
                        <p className="text-xs text-gray-400 mt-2 text-center">Tip: Click on any country on the map to view its detailed historical data.</p>
                    </div>
                </div>

                <div className="w-full lg:w-[450px] flex flex-col gap-4 shrink-0">
                    <div className="bg-white p-5 rounded-xl shadow border border-gray-100 shrink-0">
                        <h2 className="text-xl font-bold text-gray-800 border-b pb-3 mb-4">Country Details</h2>

                        <div className="mb-5">
                            <label className="block text-sm font-semibold text-gray-600 mb-1">Selected Country/Region:</label>
                            <select 
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                                <p className="text-[11px] text-gray-500 uppercase font-bold tracking-wider">Avg Temp ({selectedYear})</p>
                                <p className="text-2xl font-bold text-gray-800">
                                    {currentTemp !== undefined ? currentTemp.toFixed(2) + ' °C' : 'N/A'}
                                </p>
                            </div>
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                                <p className="text-[11px] text-gray-500 uppercase font-bold tracking-wider">vs Prev Year</p>
                                <p className={`text-xl font-bold ${diffColor}`}>{diffStr}</p>
                            </div>
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                                <p className="text-[11px] text-gray-500 uppercase font-bold tracking-wider">All-Time High</p>
                                <p className="text-lg font-bold text-red-600">
                                    {currentCountry && currentCountry.allTimeHigh !== -Infinity ? currentCountry.allTimeHigh.toFixed(2) + ' °C' : 'N/A'}
                                </p>
                            </div>
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                                <p className="text-[11px] text-gray-500 uppercase font-bold tracking-wider">All-Time Low</p>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col flex-1 min-h-[200px]">
                        <h3 className="text-sm font-bold text-gray-700 mb-2">Historical Trend (Yearly Average)</h3>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col flex-1 min-h-[200px]">
                        <h3 className="text-sm font-bold text-gray-700 mb-2">Monthly Distribution ({selectedYear})</h3>
                        <div className="flex-1 relative w-full h-full min-h-[150px]">
            ...
            {compareCountryA && !compareCountryA.startsWith('ENTITY_') && compareCountryB && !compareCountryB.startsWith('ENTITY_') && (
                <div className="bg-white p-6 rounded-xl shadow border border-gray-100 mt-4 flex flex-col gap-6">
                    <div className="flex justify-between items-center border-b pb-4">
                        <h2 className="text-2xl font-bold text-gray-800">Compare Countries</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="flex flex-col gap-4 bg-gray-50 p-6 rounded-xl border border-gray-200">
                            <div className="flex flex-col gap-2 border-b border-gray-200 pb-3">
                                <div className="flex items-center gap-2">
                                    <label className="text-sm font-bold text-gray-600 min-w-[70px]">Country 1:</label>
                                    <select 
                                        value={compareCountryA || ''} 
                                        onChange={e => setCompareCountryA(e.target.value)}
                                        className="border-gray-300 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 py-1.5 px-3 border bg-white cursor-pointer flex-1"
                                    >
                                        {state.countries.map(c => <option key={c.code} value={c.code}>{c.entity}</option>)}
                                    </select>
                                </div>
                                <div className="flex items-center gap-2">
                                    <label className="text-sm font-bold text-gray-600 min-w-[70px]">Year 1:</label>
                                    <select 
                                        value={compareYearA || ''} 
                                        onChange={e => setCompareYearA(parseInt(e.target.value))}
                                        className="border-gray-300 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 py-1.5 px-3 border bg-white cursor-pointer w-32"
                                    >
                                        {state.years.map(y => <option key={y} value={y}>{y}</option>)}
                                    </select>
                                </div>
                            ...
                            <div className="flex items-center gap-4">
                                <div 
                                    className="w-20 h-14 bg-gray-200 rounded shadow flex items-center justify-center overflow-hidden bg-cover bg-center"
                                    style={{ backgroundImage: flagA ? `url('${flagA}')` : 'none' }}
                                ></div>
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-800">{cA ? cA.entity : '-'}</h3>
                                    <p className="text-sm text-gray-500 font-semibold">Selection 1 ({compareYearA})</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 mt-2">
                                <div>
                                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Avg Temp</p>
                                    <p className="text-2xl font-bold text-gray-800">
                                        {cA && compareYearA && cA.yearly[compareYearA] !== undefined ? cA.yearly[compareYearA].toFixed(2) + ' °C' : 'N/A'}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">All-Time High</p>
                        ...
                        <div className="flex flex-col gap-4 bg-gray-50 p-6 rounded-xl border border-gray-200">
                            <div className="flex flex-col gap-2 border-b border-gray-200 pb-3">
                                <div className="flex items-center gap-2">
                                    <label className="text-sm font-bold text-gray-600 min-w-[70px]">Country 2:</label>
                                    <select 
                                ...
                                <div className="flex items-center gap-2">
                                    <label className="text-sm font-bold text-gray-600 min-w-[70px]">Year 2:</label>
                                    <select 
                                        value={compareYearB || ''} 
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                ></div>
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-800">{cB ? cB.entity : '-'}</h3>
                                    <p className="text-sm text-gray-500 font-semibold">Selection 2 ({compareYearB})</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 mt-2">
                                <div>
